...
**Arm EL0 direct-read path**

- On AArch64, the bundle APIs request direct userspace PMU access when opening perf events and use the perf mmap page to read counters with `mrs pmccntr_el0` / `mrs pmxevcntr_el0` when the kernel allows it.
- If direct userspace PMU access is unavailable, the plugin automatically falls back to `perf_evsel__read()`.
- On Linux systems that support this path, direct reads generally require perf permissions such as `kernel.perf_event_paranoid=1` and Arm userspace PMU access, for example `kernel.perf_user_access=1` where that sysctl is available.
- To verify which read path is used while testing, set `PMUV3_DEBUG_READ_PATH=1` before running an instrumented program. The plugin prints whether each event uses the direct EL0 read path or the `perf_evsel__read()` fallback path.
