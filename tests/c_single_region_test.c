#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>

#include "pmuv3_plugin_bundle.h"
#include "pmuv3_processing.h"

static volatile uint64_t g_sink;

static void work_sum(unsigned long iters)
{
    uint64_t acc = 0x123456789abcdef0ULL;

    for (unsigned long i = 0; i < iters; i++) {
        acc += i;
        acc ^= acc << 11;
        acc ^= acc >> 7;
    }

    g_sink = acc;
}

int main(int argc, char **argv)
{
    int bundle = 4;
    unsigned long iters = 100000000UL;

    if (argc > 1)
        bundle = atoi(argv[1]);
    if (argc > 2)
        iters = strtoul(argv[2], NULL, 0);

    if (pmuv3_bundle_init(bundle) != 0)
        return 1;

    uint64_t idx = get_next_index();
    get_start_count(&count_data, "c_single_work_sum", idx);
    work_sum(iters);
    get_end_count(&count_data, "c_single_work_sum", idx);

    for (uint64_t i = 0; i < num_events; i++) {
        printf("C_SINGLE,%s,%" PRIu64 "\n", event_names[i],
               event_counts[idx].end_cnt[i] - event_counts[idx].start_cnt[i]);
    }

    process_single_chunk(bundle);
    shutdown_resources();

    printf("C_SINGLE_DONE bundle=%d sink=%" PRIu64 "\n", bundle, g_sink);
    return 0;
}
